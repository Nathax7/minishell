...
/*   Updated: 2025/04/10 15:09:34 by nagaudey         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	open_infile(t_cmd *cmd, char *infile)
{
	...
	if (cmd->infile == -1)
		free_parent(cmd, 1, NULL, "Error open infile");
}
...
void	open_outfile(t_cmd *cmd, char *outfile, int mode)
{
	if (access(outfile, F_OK) == 0)
	{
		if (access(outfile, W_OK) == -1)
			free_parent(cmd, 1, NULL, outfile);
	}
	if (mode == 0)
	...
	if (cmd->outfile == -1)
		free_parent(cmd, 1, NULL, "Error open outfile");
}
